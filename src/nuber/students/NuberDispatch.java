...
public class NuberDispatch {
	...
	private final ConcurrentLinkedQueue<Driver> idleDrivers;
	private final HashMap<String, NuberRegion> regions;
	private final ExecutorService bookingService;
	...
	public NuberDispatch(HashMap<String, Integer> regionInfo, boolean logEvents)
	{
		...
		this.bookingService = Executors.newCachedThreadPool();
	...
	public Future<BookingResult> bookPassenger(Passenger passenger, String region) {
		NuberRegion reg = regions.get(region);
		if (reg == null || reg.isShuttingDown()) {
			return CompletableFuture.completedFuture(null);
		}
		
		return bookingService.submit(() -> {
			Driver driver = getDriver();
			if (driver == null) {
				// No driver available, could implement a waiting mechanism or reject the booking
				return new BookingResult(false, "No available driver");
			}
			// Simulate booking processing
			Booking booking = new Booking(passenger, driver, region);
			reg.addBooking(booking);
			logEvent(booking, "Passenger booked");
